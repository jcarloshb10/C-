...
int j,i;
void menuprincipal();
/*void menuvectores();//AGREGUÉ*/ 
void menuvectores(double v[MAXF],int &apos,int &j);
void menucrearvector(double v[MAXF],int &apos,int &j);
void vectordesdeteclado(double v[MAXF],int &apos,int &j);
void imprimirvector(double v[MAXF],int &apos,int &j);
...
void menuadicionar(double amatriz[MAXF][MAXC],int &afil,int &acol);
void adicionarfila(double amatriz[MAXF][MAXC],int &afil,int &acol);
...
void menuprincipal()
{
	...
	int afil=0,acol=0,bfil=0,bcol=0,apos=0,j;
	system("cls");
	cout
...
//	double v[];
	...
	{
		case 'a':
			menucrearvector(v,apos,j);
		break;
		case 'b':
			imprimirvector(v,apos,j);
		break;
		case 'c':
...
void menumatrices(double amatriz[MAXF][MAXC],int &afil,int &acol)
{
	...
	switch(opcion[0])
	{
		...
		break;
		case 'e':
			menuadicionar(amatriz,afil,acol);
		break;
		case 'f':
...
void adicionarfila(double amatriz[MAXF][MAXC],int &afil,int &acol)
{
	...
	{
		case 's':
			adicionarfila(amatriz,afil,acol);
		break;
		case 'n':
			menumatrices(amatriz,afil,acol);
		break;
...
void imprimirvector(double v[MAXF],int &apos,int &j)  //ERROR DE QUE NO INGRESA VECTOR Y LE DA MOSTRAR Y APARECE 0-0-0-0-0-0.. && CUANDO CREA VECTOR, LUEGO VUELVE AL MENU Y YA LE DA MOSTRAR VECTOR AHÍ YA APARECE COMO NO HA INGRESDADO NINGUN VECTOR! 
{
	system("cls");
	int i;
	if(j==0)
