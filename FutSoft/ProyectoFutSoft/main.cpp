...
int cont7=0,cont6=0,amistoso=0,cont9=0;  
...
int main() 
{
	system("mode con:cols=102lines=60");
	system("cls");
...
void menu()
{
	...
	for(int i=0;i<15;i++)
	{ 
		nomina_e[i] = new equipos[cont_equipo];
...
void ingr_nombre_credito()
{
	...
	if(cont_registro==1)
	{
		...
		gotoxy(4,6);cout<<"IDENTIFICACI"<<(char)224<<"N:";
	...
	else
	{
		...
		gotoxy(6,5);cout<<"YA HA INGRESADO SU CR"<<(char)144<<"DITO: $ "<<paux->credito;
		getch();
	}
...
void alineaciones()
{
	...
	if(cont_registro!=0)
	{
		...
		gotoxy(3,2);cout<<"DIGITE ALINEACION DEL EQUIPO:";gotoxy(34,2);cin>>aux;
		
		if(isdigit(aux[0]))
		{
			formacion=atoi(aux);
			if(formacion>0 && formacion <5)
			{
				cin.ignore();
...
void ingr_nomina_entrenador()
{
	...
	gotoxy(28,2);cin.getline(nomina_e[0][cont_equipo-1].nom_team,49,'\n');
	...
	{
		gotoxy(4,6+i);cout<<"NOMBRE DEL DEFENSA #"<<i+1;
	...
	{
		gotoxy(4,11+i);cout<<"NOMBRE DEL MEDIO #"<<i+1;
		gotoxy(28,11+i);cin.getline(nomina_e[cont][cont_equipo-1].jugadores,49,'\n');
		cont++;
		gotoxy(50,3);cout<<"-> HAY "<<cont<<" JUGADOR(ES)";
	}
	for(i=0;i<nomina_e[0][cont_equipo-1].del;i++)
	{
		gotoxy(4,17+i);cout<<"NOMBRE DEL DELANTERO #"<<i+1;
		gotoxy(28,17+i);cin.getline(nomina_e[cont][cont_equipo-1].jugadores,49,'\n');
		cont++;
		gotoxy(50,3);cout<<"-> HAY "<<cont<<" JUGADOR(ES)";
		if(cont==11)
		{
			gotoxy(53,4);cout<<"EQUIPO COMPLETO";
		}
	}
...
void mostrar_nomina_tecnico()
{
	int i,j,cont2=0,pos=0,cont_let=0,aux=0;
	...
	else
	{
		if(cont_registro==0 && cont_equipo==0)
		{
			gotoxy(4,15);cout<<" NO HA INGRESADO UN USUARIO, NI EQUIPO.";
		}
		else
		{
			if(cont_registro==0)
			{
				gotoxy(9,15);cout<<" NO HA INGRESADO UN USUARIO.";
			}
			if(cont_equipo==0)
			{
				gotoxy(9,15);cout<<"NO HA CREADO UN EQUIPO.";
			}			
		}
...
void apuestas()
{
	...
	if(cont_registro>0 && cont_equipo>=2)  
	{
...
void apuesta_campeon_liga()
{
	int i,cont3=0,equipo_apostado=0,gol_local,gol_visitante;  
	
	
	colocar_ventanas(1,6,102,32,1,15);
	colocar_borde(1,1,102,54);
	gotoxy(41,3);cout<<"APUESTAS POR CAMPEON DE LIGA";	
	...
	if(paux->credito >=10)
	{
		...
		{
			gotoxy(3,5);cout<<"SOLO HAY "<<cont_equipo<<" EQUIPO(S), MINIMO 4";
		}

	}
	else
	{
			gotoxy(3,5);cout<<"CREDITO INSUFICIENTE PARA JUGAR ";
			getch();
			menu();
	}
...
void estadistica_liga()
{
	...
	gotoxy(18,3);cout<<"GOLES A FAVOR";
	gotoxy(36,3);cout<<"GOLES EN CONTRA";
	gotoxy(53,3);cout<<"+/- GOL";
	int y =0;
	
	for(int i =0; i< cont_equipo;i++)
	{
		gotoxy(5,5+y);cout<<nomina_e[0][i].nom_team;
		gotoxy(18,5+y);cout<<paux->gol_favor[cont9][i];  
		gotoxy(36,5+y);cout<<paux->gol_contra[cont9][i];   
		paux->diferencia_gol[cont9][i]=paux->gol_favor[cont9][i]-paux->gol_contra[cont9][i];  
		gotoxy(53,5+y);cout<<paux->diferencia_gol[cont9][i];   
	...
	for(int i =0; i<cont_equipo ;i++)
	{
		gotoxy(10,11+y);cout<<nomina_e[0][i].nom_team<<" TIENE "<<paux->puntos[cont9][i]<<" PUNTOS ";   
		y++;
		if (paux->puntos[cont9][i+1] > mayor)   
		{
			mayor = paux->puntos[cont9][i+1];   
			ganador = i+1;
		} 
		if(paux->puntos[cont9][i+1] == mayor)
		{
			if(paux->diferencia_gol[cont9][i+1]>paux->diferencia_gol[cont9][0])
			{
				mayor = paux->puntos[cont9][i+1];
				ganador = i+1;	
			}
